...
def plot_all_ligand_properties(json_files, candms, expts):
    ...
    for i in json_files:
        ...
        print(x, y, c, m)
        ax.scatter(x, y, c=c, edgecolors="k", marker=m, alpha=1.0, s=120)
    ...
    ax.set_ylabel(r"long axis deviation [$\mathrm{\AA}$]", fontsize=16)
    ax.set_xlim(0.0, 8)
    ax.set_ylim(0.1, 2.1)
...
def plot_MM_vs_AR(
    json_files, candms, expts, full=False, short=False, grouped=False
):
    _figure_path = "figures"
    ...
    for i in json_files:
        ...
        for face in cage_set_data[dict_key]:
            stabs[face]["ar"].append(cage_set_data[dict_key][face][1])
            stabs[face]["avgMDif"].append(cage_set_data[dict_key][face][0])
    ...
    fig.savefig(
        os.path.join(_figure_path, f"all_ligand_MM_vs_AR{addon}{suffix}.pdf"),
        dpi=720,
        bbox_inches="tight",
    )
...
def plot_MM_vs_NN(
    ...
    for i in json_files:
        ...
        lname = cage_set.replace("cl1_", "")
        for face in cage_set_data[dict_key]:
            stabs[face]["nn"].append(cage_set_data[dict_key][face][2])
            stabs[face]["avgMDif"].append(cage_set_data[dict_key][face][0])
            stabs[face]["name"].append(
                (
                    convert_lig_names_from_cage(lname, as_int=True),
    ...
    ax.legend(fontsize=16, ncol=ncol)
    fig.savefig(
        os.path.join(_figure_path, f"all_ligand_MM_vs_NN{addon}{suffix}.pdf"),
        dpi=720,
        bbox_inches="tight",
    )
...
def main():
    first_line = "Usage: plot_ligand_properties.py expt_lib_file"
