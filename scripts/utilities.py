...
def get_lowest_energy_conformers(
    ...
    for lig in org_ligs:
        ...
        if os.path.exists(final_filename_):
            continue
        print(f"......calculating lowest energy conformer of {ligand_name_}")
...
def get_planar_conformer(molecule, N=100):
    cids, confs = build_conformers(mol=molecule, N=N, ETKDG_version="v3")
    ...
    for cid in cids:
        ...
        new_molecule = update_from_rdkit_conf(
            stk_mol=new_molecule, rdk_mol=confs, conf_id=cid
        )
        ...
        if plane_dev < min_plane_dev:
            ...
            molecule = update_from_rdkit_conf(
                stk_mol=molecule, rdk_mol=confs, conf_id=min_cid
            )
...
def planarfy(ligands):
    ...
    for ligand in ligands:
        ...
        if os.path.exists(planar_file):
            opt_lig = ligands[ligand].with_structure_from_file(planar_file)
        else:
