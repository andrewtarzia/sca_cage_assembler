...
"""
    ...
    output_files = [
        i
        for i in glob.glob(os.path.join(dft_directory, "*out"))
        if "slurm" not in i
    ]
    ...
    for fi in output_files:
        ...
        if check_convergence(fi):
            ...
            systems_data[system][(cutoff, rel_cutoff)] = (energy, forces)
        else:
    ...
    for syst in systems_data:
        ...
        fig.tight_layout()
        fig.savefig(
            os.path.join(_figure_path, f"{syst}_at{inital_rel_cutoff}rel.pdf"),
            dpi=720,
            bbox_inches="tight",
        )
    ...
    for syst in systems_data:
        ...
        fig.tight_layout()
        fig.savefig(
            os.path.join(_figure_path, f"{syst}_at{chosen_cutoff}ry.pdf"),
            dpi=720,
            bbox_inches="tight",
        )
