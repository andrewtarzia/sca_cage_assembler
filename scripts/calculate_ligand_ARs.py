...
def plot_parities(C_data, Br_data, N_data, output_name):
    ...
    for lig in C_data:
        ...
        y2 = brar
        ax.scatter(
            x, y1, c="gray", edgecolors="k", marker="o", alpha=1.0, s=120
        )
        ax.scatter(
            x, y2, c="sandybrown", edgecolors="k", marker="o", alpha=1.0, s=120
        )
    ...
    if "ARs" in output_name:
        ...
        ax.plot(np.linspace(0, 8), np.linspace(0, 8), "--", c="k", lw=2)
        ax.set_xlabel(r"N-N difference [$\mathrm{\AA}$]", fontsize=16)
        ax.set_ylabel(r"test difference [$\mathrm{\AA}$]", fontsize=16)
        ax.set_xlim(0.0, 8)
        ax.set_ylim(0.0, 8)
    ax.legend(fontsize=16, ncol=2)
    fig.savefig(output_name, dpi=720, bbox_inches="tight")
    plt.close()
...
def main():
    first_line = "Usage: calculate_ligand_ARs.py"
...
{first_line}
    ...
    for l_file in glob("*_planar.mol"):
        ...
        nar = calculate_binding_AR(mol=reacted_planar_mol, atom_ids=n_atom_ids)
