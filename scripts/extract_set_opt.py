...
"""
    ...
    all_cage_structures = glob.glob(
        os.path.join(cage_directory, f"C_{cage_set}_*optc.mol")
    )
    ...
    for lig_mol in sorted(all_cage_structures):
        ...
        opt_xyz_traj = os.path.join(
            dft_directory, opt_file.replace(".mol", "_opt-pos-1.xyz")
        )
        opt_xyz_file = os.path.join(
            dft_directory, opt_file.replace(".mol", "_opt-pos-1_fin.xyz")
        )
