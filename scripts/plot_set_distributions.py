...
def distribution_plot(df, col_name, sets_to_plot):
    ...
    yprops = {
        "octop": (r"min. $q_{\mathrm{oct}}$", (None, None), "min"),
        ...
        "lsesum": (r"sum strain energy [kJmol$^{-1}$]", (None, None), "max"),
        "minitors": (r"min. imine torsion [degrees]", (None, None), "min"),
        "maxcrplan": (
        ...
        "porediam": (r"pore diameter [$\mathrm{\AA}$]", (None, None), "min"),
        "relformatione": (
            r"rel. formation energy [kJmol$^{-1}$]",
            (-10, 1000),
            "max",
        ),
        "maxintangledev": (
    ...
    for cageset in sets_to_plot:
        ...
        if cageset == "cl1_quad2_1":
            ...
            for i in formed_symm:
                print(i)
                stabilisation_energy = i[1] - min(all_energies)
    ...
    for cageset in sets_to_plot:
        ...
        for i in xtb_set_values:
            if xtb_set_values[i][1] is True:
                formed_pair = (xtb_set_values[i][0], dft_set_values[i][0])
            all_energies.append((xtb_set_values[i][0], dft_set_values[i][0]))

        minx_energy = min([i[0] for i in all_energies])
        miny_energy = min([i[1] for i in all_energies])
        ax.scatter(
            x=[(i[0] - minx_energy) * 2625.5 for i in all_energies],
            y=[(i[1] - miny_energy) * 2625.5 for i in all_energies],
            c="gray",
            s=40,
            alpha=0.5,
        )
...
def main():
    first_line = "Usage: plot_set_distributions.py"
