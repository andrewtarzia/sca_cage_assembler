...
def main():
    first_line = "Usage: report_on_construction.py"
...
{first_line}
    ...
    for cage_set_file in cs_files:
        ...
        _output_string += "-----------------\n"
        _output_string += f"{cage_set} from {cage_set_file} report:\n"
        with open(cage_set_file) as f:
            cs_data = json.load(f)
        ...
        for cage_name in cs_data:
            ...
            if os.path.exists(opt_file):
                ...
                stk.BuildingBlock.init_from_file(opt_file).write(
                    os.path.join(
                        _output_directory, opt_file.replace(".mol", ".pdb")
                    )
                )
            else:
            ...
            if cage_data["optimized"]:
                count_optimized += 1
                _output_string += "2) optimized!\n"
            else:
                count_unoptimized += 1
                _output_string += "2) not optimized!\n"
