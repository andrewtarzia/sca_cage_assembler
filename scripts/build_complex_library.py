...
from utilities import get_atom_distance, read_lib
...
def build_complexes(complexes, ligand_directory):
    for name in complexes:
        ...
        ligand_fg_factories = [
            custom_fg_factories(i) for i in ["CNBr_metal", "CNC_metal"]
        ]
        ...
        complex = optimize_SCA_complex(
            complex=complex, name=name, dict=comp, metal_FFs=custom_metal_FFs
        )
    ...
    for name in complexes:
        ...
        for output in outputs:
            ...
            for pair in atomic_number_pairs:
                ...
                print(f"file: {output}, pair: {pair}, avg. dists: {avgd}")
