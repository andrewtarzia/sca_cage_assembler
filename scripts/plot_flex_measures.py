...
def plot_dists(json_files, data_name, xlabel, xprops):
    ...
    for i, _file in enumerate(json_files):
        lig_name = _file.replace("_flex_measure.json", "").split("_")[1]
        with open(_file) as f:
            data = json.load(f)
        measures = data[data_name]
        ax[i].text(xprops[0] + 0.1, 0.0, lig_name, fontsize=16)
        ax[i].hist(
            x=measures,
            bins=xbins,
            density=True,
    ...
    fig.tight_layout()
    fig.savefig(f"flex_{data_name}_dists.pdf", dpi=720, bbox_inches="tight")
    plt.close()
...
def plot_ladev_planedev(json_files):
    ...
    for i, _file in enumerate(json_files):
        with open(_file) as f:
            data = json.load(f)
        x = abs(
            max(data["long_axis_distances"]) - min(data["long_axis_distances"])
        )
        y = abs(max(data["plane_deviations"]) - min(data["plane_deviations"]))
    ...
    fig.tight_layout()
    fig.savefig("flex_comp.pdf", dpi=720, bbox_inches="tight")
    plt.close()
...
def plot_vectangle_energy_permol(json_files):
    ...
    for i, _file in enumerate(json_files):
        ...
        y_measures = [
            (i - min(data["energies"])) * 2625.5 for i in data["energies"]
        ]
        ax.scatter(
            x=x_measures,
            y=y_measures,
        ...
        ax.tick_params(labelsize=16, bottom=True)
        ax.set_xlabel("vector angle [degrees]", fontsize=16)
        ...
        fig.tight_layout()
        fig.savefig(
            f"{lig_name}_flex_torsion_energy.pdf", dpi=720, bbox_inches="tight"
        )
        plt.close()
...
def plot_ladev_angle(json_files):
    ...
    for i, _file in enumerate(json_files):
        ...
        y = abs(max(data["vector_angle"]) - min(data["vector_angle"]))
    ...
    fig.tight_layout()
    fig.savefig("flex_ladev_angle.pdf", dpi=720, bbox_inches="tight")
    plt.close()


def main():
    first_line = "Usage: plot_flex_measures.py"
