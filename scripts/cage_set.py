...
class CageSet:
    ...
    def __init__(
        ...
        self.ligand_measures_file = f"{self.name}_ligand_measures.json"
        ...
        self.ligand_aspect_difference = self._get_ligand_ABS(ligand_dir)
    ...
    def _get_ideal_pore_size(self, ligand_dir):
        ...
        tet_linker = self._load_ligand(
            ligand_name=self.cage_set_dict["tetratopic"], ligand_dir=ligand_dir
        )

        planar_file = join(
            ligand_dir, f"{self.cage_set_dict['tetratopic']}_planar.mol"
        )
    ...
    def _get_face_properties(self, ligand_dir, long_opt):
        ...
        for face_prop_file in face_prop_files:
            ...
            face_type = face_prop_file.replace(face_dir, "").split("_")[4]
            with open(face_prop_file) as f:
                data = json.load(f)
    ...
    def get_max_face_interior_angle_dev(self, cage_name):
        ...
        return max(
            [
                abs(360 - sum(C_data["cl_prop"][i].values()))
    ...
    def load_properties(self):
        ...
        if exists(self.properties_file):
            ...
            raise FileNotFoundError(f"{self.properties_file} does not exist")
    ...
    def plot_Y(self, data, ylabel, filename, ylim=None):
        ...
        for i, name in enumerate(data):
            ...
            x_pos_list.append(X)
            ax.scatter(
                X, data[name], c=C, edgecolors="k", marker=M, alpha=1.0, s=180
            )
    ...
    def plot_Y_C(
        ...
        ax.scatter(xs, ys, c=cs, edgecolors="k", marker=M, alpha=1.0, s=180)
