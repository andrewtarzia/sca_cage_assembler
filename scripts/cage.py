...
class Cage:
    ...
    def save_bb_vector_xyzs(self, structure_file):
        ...
        def get_ligand_vectors(mol, metal_atom_nos):
            ...
            ligand_bb_atom_ids = {
                i: bb_atom_ids[i] for i in bb_atom_ids if i not in metal_bbs
            }
            ...
            for bb in ligand_bb_atom_ids:
                bb_bonders = [
                    i for i in bonder_atom_ids if i in ligand_bb_atom_ids[bb]
                ]
                for atom_ids in combinations(bb_bonders, r=2):
                    c1_pos = tuple(mol.get_atomic_positions(atom_ids[0]))[0]
                    c2_pos = tuple(mol.get_atomic_positions(atom_ids[1]))[0]
        ...
        def get_metal_vectors(mol, metal_atom_nos):
            ...
            query_ids = get_query_atom_ids(smarts, rdkit_mol)
            for atom_ids in query_ids:
                n1_pos = tuple(mol.get_atomic_positions(atom_ids[0]))[0]
                n2_pos = tuple(mol.get_atomic_positions(atom_ids[3]))[0]
                pts = [
    ...
    def optimize(self, free_e, step_size, distance_cut, scale_steps):
        ...
        try:
            ...
            opt_output_file = join(
                f"cage_opt_{self.name}_xtb", "optimization_1.output"
            )
    ...
    def calculate_formation_energy(
        ...
        for comp in components:
            if comp in ["mprec", "mpreclig"]:
                ...
                if components[comp]["smiles"] is not None:
                    ...
                    if exists(opt_file):
                        temp_mol = temp_mol.with_structure_from_file(opt_file)
                    else:
                    ...
                    if exists(low_e_file):
                        ...
                        conf_folder = f"{components[comp]['name']}_confs/"
                ...
                low_e_files = [i for i in low_e_lig_filenames if lig_name in i]
        ...
        prod_ey = sum(
            [
                ...
                for i in components
                if components[i]["product"] is True
            ]
        )
        react_ey = sum(
            [
                components[i]["total_e"] * components[i]["count"]
                for i in components
                if components[i]["product"] is False
            ]
        )
        fe = prod_ey - react_ey
        self.fe_data = fe
    ...
    def analyze_cube_shape(self):
        ...
        m_structure = stk.BuildingBlock.init_from_file(f"{self.name}_M.mol")
    ...
    def analyze_metal_strain(self):
        ...
        if not exists(f"{self.op_file}.json"):
            ...
            for metal in pres_atm_no:
                op_set = get_order_values(
                    mol=self.cage, metal=metal, per_site=True
                )
                op_res[metal] = op_set
        ...
        # Get data.
        with open(f"{self.op_file}.json") as f:
