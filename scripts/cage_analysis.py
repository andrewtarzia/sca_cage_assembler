...
def plot_heatmap_X_vs_Y(
    ...
    col_names = {
        i: (j[xname], convert_lig_names_from_cage("_".join(i.split("_")[1:])))
        for i, j in data.items()
    }
    col_names = {
        i: j for i, j in sorted(col_names.items(), key=lambda item: item[1][0])
    }
    ...
    im, cbar = heatmap(
        ...
        col_labels=[f"{i[1]}: {round(i[0], 2)}" for i in col_names.values()],
        ax=ax,
        cmap="Blues",
        cbarlabel=ylabel,
    )
    annotate_heatmap(
        im=im, valfmt="{x:.1f}", fontsize=8, na_points=tuple(na_points)
    )
...
def plot_4Dheatmap_X_vs_Y(
    ...
    for cagesetname in ydata:
        _ids = [i for i in range(len(Xnames)) if Xnames[i] == cagesetname]
        for _id in _ids:
...
def cage_set_properties(cage_set):
    ...
    if exists(cage_set.measures_file):
        with open(cage_set.measures_file) as f:
...
def write_csv(cage_sets, experimentals):
    ...
    for cage_set in cage_sets:
        ...
        cage_set_data[cage_set.name] = {
            ...
            "relformatione": start_at_0(data_dict=measures["formatione"]),
            "maxintangledev": measures["maxintangledev"],
        }

    tested_cage_sets = [
        key for key in cage_set_data if f"_{key}_" in "__".join(experimentals)
    ]
    set_columns = [
        "cageset",
        "symmetry",
